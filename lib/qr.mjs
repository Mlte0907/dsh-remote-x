...
function buildCodewords(bytes, version) {
  ...
  // 3) 填充字节 0xEC / 0x11 交替
  //    （packBits 只到最后一个数据位所在的字节，必须显式扩容后再填，
  //     否则越界写入会被 TypedArray 静默丢弃，尾部留下全 0）
  ...
  for (let i = packed.length; i < totalData; i += 1) {
    payload[i] = (i - packed.length) % 2 === 0 ? 0xec : 0x11
  }

  // 4) 分块：短块在前，长块比短块多 1 个数据码字
...
function formatBits(mask) {
  const data = (0b00 << 3) | mask // 纠错等级 M = 00
  let rem = data
  for (let i = 0; i < 10; i += 1) rem = ((rem << 1) ^ ((rem >>> 9) * 0x537)) & 0x7fff
...
function versionBits(version) {
  let rem = version
  for (let i = 0; i < 12; i += 1) rem = ((rem << 1) ^ ((rem >>> 11) * 0x1f25)) & 0x3ffff
...
function penalty(modules, size) {
  ...
  // 规则 3：疑似定位图案的 1:1:3:1:1 序列（四个方向共 4 种 11 模块组合）
  const seq1 = [1, 0, 1, 1, 1, 0, 1, 0, 0, 0, 0]
...
export function encodeQr(text) {
  ...
  // 顺序很重要：先画贯穿全图的定时图案，再用定位图案覆盖掉它的两端
  // （定位图案在 (6,1)/(6,3)/(6,5) 等位置是深色的，与定时图案的交替相反）。
  // 反过来画会把定位图案的角啃掉，码就扫不出来了。
  ...
  // 固定深色模块：必须登记为功能模块，否则数据填充会往这里写一位数据，
  // 之后又被格式信息覆盖成深色 —— 该位数据被吞掉，码字流整体错位一位。
  setFn(8, size - 8, true)

  // 数据码字：两列一组自右向左蛇形填充，跳过第 6 列
  let bitIndex = 0
  const totalBits = codewords.length * 8
  for (let right = size - 1; right >= 1; right -= 2) {
    // 跳过第 6 列的竖直定时图案：把 right 改写成 5，后续步长自然落到 3/1，
    // 否则会重复扫第 4 列并漏掉最左侧的几列（右侧列先填，短内容时看不出来，
    // 数据一多就会把码字写丢）。
    if (right === 6) right = 5
