...
//   - 放行只认做过值校验的凭据：?key= / /k/ 与 accessKey 恒时比较，?token= / /t/
//     对照最新登录口令，remote-x-key cookie 按值比对；认证通过的请求另获代理
//     自签的 remote-x-session cookie，之后凭它免输续行（见 gate）
//   - dsh-auth-* cookie 只代表"上游可能已登录"，仅用于省掉一次口令代换；
//     它可被任意伪造，从不作为放行依据
//
...
import { TOKEN_CACHE_MS, LOG_TAIL_BYTES, REMEMBER_COOKIE_MAX_AGE, UPSTREAM_HEADER_TIMEOUT_MS, BACKOFF_SCHEDULE } from './constants.mjs'
...
const SESSION_COOKIE = 'remote-x-session'
...
/**
 * 解析 Cookie 头为 `名称 → 值列表`。同名多值全保留：任一值通过校验即算持有，
 * 排在前面的伪造值顶不掉真实值。没有 `=` 的段跳过。
 * @param {string | undefined} header 入站 Cookie 头
 * @returns {Record<string, string[]>} 名称到值列表的映射（无原型对象）
 */
function parseCookies(header) {
  ...
  for (const part of header.split(';')) {
    ...
    if (name === '') continue
    ;(out[name] ??= []).push(part.slice(i + 1).trim())
  }
  return out
}
...
/**
 * 会话签名密钥默认存放处（0600）。跨代理重启存活，设备无需重新输口令。
 * @returns {string} 密钥文件路径
 */
function defaultSessionSecretFile() {
  return path.join(homedir(), '.dsh', 'remote-x-session.key')
}

/**
 * 读取会话签名密钥；缺失或不可读时生成新值并落盘（0600）。
 * 落盘失败不阻断：仍返回本次生成的密钥，本进程签发/校验照常，
 * 只损失跨重启的会话存活。
 * @param {string} file 密钥文件路径
 * @param {(info: object) => void} [onError] 落盘失败回调
 * @returns {string} 64 位十六进制密钥
 */
function loadSessionSecret(file, onError) {
  try {
  ...
  try {
    ...
    onError?.({ kind: 'session-secret', error })
  }
  return secret
}

/**
 * 签发会话 cookie 值：`<随机 nonce>.<过期时刻(ms)>.<HMAC-SHA256>`，
 * 签名覆盖前两段；值本身不含任何访问口令。
 * @param {string} secret 签名密钥
 * @param {number} now 当前时刻（ms）
 * @returns {string} cookie 值
 */
function mintSessionValue(secret, now) {
  const payload = `${randomBytes(16).toString('base64url')}.${now + REMEMBER_COOKIE_MAX_AGE * 1000}`
  return `${payload}.${createHmac('sha256', secret).update(payload).digest('base64url')}`
}

/**
 * 校验会话 cookie 值：段数、有效期、签名（恒时比较）三项全过才成立，
 * 伪造或篡改任一段都在签名比对处失败。
 * @param {string} secret 签名密钥
 * @param {string} value cookie 值
 * @returns {boolean} 是否为本代理签发且未过期
 */
function verifySessionValue(secret, value) {
  ...
  return timingSafeEqualStr(parts[2], expected)
}
...
/** 插件状态文件默认位置（与 src/index.ts 共用，0600）。 */
function defaultStateFile() {
  return path.join(homedir(), '.dsh', 'remote-x-state.json')
}

/**
 * 读 6 位登录口令。文件缺失、字段缺失或形状不符一律返回 undefined ——
 * 登录面整体关闭，门禁退回纯文本 401（失败关闭）。每次尝试都重读：
 * 设置页重新生成口令后无需重启代理即生效。
 * @param {string} file 状态文件路径
 * @returns {string | undefined} 6 位数字口令
 */
function readLoginPassword(file) {
  try {
    const raw = JSON.parse(readFileSync(file, 'utf8'))
    const login = raw?.login
    return typeof login === 'string' && /^\d{6}$/.test(login) ? login : undefined
  } catch {
    return undefined
  }
}
...
/**
 * 回跳地址只接受站内单斜杠相对路径：`//host`、反斜杠、换行（头注入）一律退回 `/`。
 * @param {unknown} value 候选回跳地址
 * @returns {string} 安全的 Location / 表单回填值
 */
function safeRedir(value) {
...
/**
 * 登录页 HTML：单表单、零外链资源，隧道弱网下也能秒开；深浅主题跟随系统。
 * @param {string} redir 登录成功后的回跳路径（须已过 safeRedir）
 * @param {string} [error] 表单上方错误提示
 * @returns {string} 完整 HTML 文档
 */
function loginPageHtml(redir, error) {
  const errorBlock = error === undefined ? '' : `<p class="err">${escapeHtml(error)}</p>`
  return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>DSH 远程登录</title>
<style>
:root { color-scheme: light dark; }
body { margin: 0; min-height: 100vh; display: flex; align-items: center; justify-content: center; font: 15px/1.6 system-ui, -apple-system, "Segoe UI", sans-serif; background: #f6f8fa; color: #1f2328; }
@media (prefers-color-scheme: dark) { body { background: #0d1117; color: #e6edf3; } }
.card { width: min(360px, calc(100vw - 40px)); padding: 32px 28px; border-radius: 16px; background: #fff; border: 1px solid #d0d7de; box-shadow: 0 8px 24px rgba(140,149,159,.15); }
@media (prefers-color-scheme: dark) { .card { background: #161b22; border-color: #30363d; } }
h1 { margin: 0 0 4px; font-size: 18px; }
.sub { margin: 0 0 20px; font-size: 13px; opacity: .65; }
label { display: block; font-size: 13px; margin-bottom: 6px; opacity: .8; }
input { width: 100%; box-sizing: border-box; padding: 10px 12px; font-size: 22px; letter-spacing: 8px; text-align: center; font-variant-numeric: tabular-nums; border: 1px solid #d0d7de; border-radius: 10px; background: transparent; color: inherit; outline: none; }
input:focus { border-color: #1a7f37; }
button { width: 100%; margin-top: 16px; padding: 11px; font-size: 15px; font-weight: 600; color: #fff; background: #1a7f37; border: 0; border-radius: 10px; cursor: pointer; }
button:active { background: #116329; }
.err { margin: 0 0 14px; padding: 9px 12px; font-size: 13px; color: #cf222e; background: rgba(242,90,90,.1); border: 1px solid rgba(207,34,46,.3); border-radius: 10px; }
.hint { margin: 14px 0 0; font-size: 12px; opacity: .55; text-align: center; }
</style>
</head>
<body>
<div class="card">
  <h1>DSH 远程登录</h1>
  <p class="sub">输入 6 位登录口令继续（设置页「远程控制」内查看）</p>
  ${errorBlock}
  <form method="post" action="${LOGIN_PATH}">
    <input type="hidden" name="redir" value="${escapeHtml(redir)}">
    <label for="p">登录口令</label>
    <input id="p" name="p" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="6" autocomplete="off" autocorrect="off" spellcheck="false" autofocus required>
    <button type="submit">登录</button>
  </form>
  <p class="hint">口令错误会逐级增加等待时间</p>
</div>
</body>
</html>`
}
...
/**
 * 读表单 POST 的请求体（登录页字段很少，上限 2KB，超限销毁连接）。
 * @param {import('node:http').IncomingMessage} req 入站请求
 * @param {number} maxBytes 字节上限
 * @returns {Promise<string>} 原始请求体
 */
 ...
 * @param {string} [options.sessionSecretFile] 会话 cookie 签名密钥文件（默认 ~/.dsh/remote-x-session.key）
 * @param {string} [options.stateFile] 插件状态文件（含 6 位登录口令；默认 ~/.dsh/remote-x-state.json）
...
export function startRemoteProxy(options) {
  ...
  // 会话签名密钥延迟到首个请求加载：缺失则生成落盘，之后进程内复用。
  let sessionSecret
  const getSessionSecret = () => (sessionSecret ??= loadSessionSecret(
    options.sessionSecretFile ?? defaultSessionSecretFile(),
    options.onError,
  ))

  // 登录口令与插件共用状态文件（每次尝试重读，设置页换号即时生效）
  const stateFile = typeof options.stateFile === 'string' ? options.stateFile : defaultStateFile()
  // 登录限速：frp 隧道的入站一律来自 127.0.0.1，按 IP 计数形同虚设 ——
  // 用全代理计数 + 指数退避（BACKOFF_SCHEDULE），连续失败逐级锁死，成功清零。
  let loginFails = 0
  let loginLockedUntil = 0

  /**
   * 登录表单 POST：读体 → 限速检查 → 恒时比较 → 签发会话 cookie。
   * 限速先于比较：锁定期内连正确口令也拒绝，防穷举且不泄露口令状态。
   * @param {import('node:http').IncomingMessage} req 入站请求
  ...
  async function handleLogin(req, res) {
    let params
    try {
      params = new URLSearchParams(await readFormBody(req, 2_048))
    } catch {
      if (!res.headersSent) res.writeHead(400, { 'content-type': 'text/html; charset=utf-8' })
      res.end(loginPageHtml('/', '请求无效'))
      return
    }
    const now = Date.now()
    if (now < loginLockedUntil) {
      const retryAfter = Math.max(1, Math.ceil((loginLockedUntil - now) / 1000))
      res.writeHead(429, { 'content-type': 'text/html; charset=utf-8', 'retry-after': String(retryAfter), 'cache-control': 'no-store' })
      res.end(loginPageHtml(safeRedir(params.get('redir')), `尝试过于频繁，请 ${retryAfter} 秒后再试`))
      return
    }
    const password = readLoginPassword(stateFile)
    if (password === undefined) {
      res.writeHead(401, { 'content-type': 'text/plain; charset=utf-8' })
      res.end('dsh-remote-x: 需要访问口令 — 在 URL 后追加 ?key=<口令>，或在设置页重新扫码')
      return
    }
    const provided = params.get('p') ?? ''
    if (!timingSafeEqualStr(provided, password)) {
      loginFails += 1
      loginLockedUntil = Date.now() + BACKOFF_SCHEDULE[Math.min(loginFails - 1, BACKOFF_SCHEDULE.length - 1)]
      const retryAfter = Math.max(1, Math.ceil((loginLockedUntil - Date.now()) / 1000))
      res.writeHead(401, { 'content-type': 'text/html; charset=utf-8', 'retry-after': String(retryAfter), 'cache-control': 'no-store' })
      res.end(loginPageHtml(safeRedir(params.get('redir')), '口令错误'))
      return
    }
    loginFails = 0
    loginLockedUntil = 0
    res.writeHead(303, {
      location: safeRedir(params.get('redir')),
      'set-cookie': `${SESSION_COOKIE}=${mintSessionValue(getSessionSecret(), Date.now())}; Path=/; HttpOnly; Max-Age=${REMEMBER_COOKIE_MAX_AGE}; SameSite=Strict`,
  ...
  /**
   * 校验请求凭据并改写 URL；未通过返回 `{ denied: true }`（入口回 401）。
   *
   * 放行条件（逐条做值校验，仅凭 cookie 名称一律不认）：
   *   1. ?key= / /k/<key>/ 与 accessKey 恒时比较相符；
   *   2. remote-x-key cookie 某个值与 accessKey 恒时比较相符（记忆设备）；
   *   3. remote-x-session cookie 某个值由本代理签发且未过期（会话设备）；
   *   4. ?token= / /t/<token>/ 与最新 dsh 登录口令相符（进程内直传优先）。
   * dsh-auth-* cookie 不参与放行：它可被任意伪造，只用于跳过口令代换。
   * @param {import('node:http').IncomingMessage} req 入站请求
   * @returns {Promise<{denied: true} | {url: string, remember: boolean, mint: boolean, sentToken: boolean}>} 门禁结果
   */
  async function gate(req) {
    let raw = req.url ?? '/'
    const cookies = parseCookies(req.headers.cookie)
    const authed = Object.keys(cookies).some(name => name.startsWith('dsh-auth-'))
    ...
    const rememberedOk = accessKey !== undefined
      && (cookies[REMEMBER_COOKIE] ?? []).some((value) => {
        try { return timingSafeEqualStr(decodeURIComponent(value), accessKey) } catch { return false }
      })
    const secret = getSessionSecret()
    const sessionOk = (cookies[SESSION_COOKIE] ?? []).some(value => verifySessionValue(secret, value))
    // 内联口令（?token= 或 /t/<token>/）：与进程内直传或日志扫描出的最新口令比较。
    // 上面三类凭据都不成立时才需要它；再没有就拒绝。
    let tokenOk = false
    if (!keyOk && !rememberedOk && !sessionOk) {
      ...
      if (provided !== undefined && provided.length > 0) {
        const latest = staticToken ?? (await readDshToken(tokenFile))
    ...
    return {
      url: out,
      remember: keyOk && !authed && !rememberedOk,
      mint: !sessionOk && (keyOk || tokenOk),
      sentToken,
    }
  }
  ...
  const server = createServer(async (req, res) => {
    ...
    // 登录表单是代理本地路由：未认证入口，不进门禁也不转发上游
    if (req.method === 'POST' && (req.url ?? '/').split('?')[0] === LOGIN_PATH) {
      handleLogin(req, res).catch((error) => {
        options.onError?.({ kind: 'login', error })
        if (!res.headersSent) res.writeHead(500, { 'content-type': 'text/plain; charset=utf-8' })
        res.end(`dsh-remote-x: login error: ${error.message}`)
      })
      return
    }
    let gated
    ...
    if (gated.denied) {
      // 浏览器整页导航且已配置登录口令：回登录页（状态仍 401，语义不变）；
      // 其余（XHR / WS / 无 Accept / 口令面关闭）保持纯文本 401。
      if (req.method === 'GET'
        && String(req.headers.accept ?? '').includes('text/html')
        && readLoginPassword(stateFile) !== undefined) {
        res.writeHead(401, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' })
        res.end(loginPageHtml(safeRedir(req.url ?? '/')))
        return
      }
      res.writeHead(401, { 'content-type': 'text/plain; charset=utf-8' })
      res.end('dsh-remote-x: 需要访问口令 — 在 URL 后追加 ?key=<口令>，或在设置页重新扫码')
    ...
    const respond = (upstreamRes) => {
      ...
      if (gated.mint) {
        outHeaders = mergeSetCookie(
          outHeaders,
          `${SESSION_COOKIE}=${mintSessionValue(getSessionSecret(), Date.now())}; Path=/; HttpOnly; Max-Age=${REMEMBER_COOKIE_MAX_AGE}; SameSite=Strict`,
        )
      }
    ...
    const dispatch = (freshConnection) => {
      const proxied = httpRequest(
        ...
        (upstreamRes) => {
          ...
          // 客户端带着对上游已失效的 dsh-auth-* cookie 时，gate 仍凭会话/记忆
          // cookie 放行，但 authed 会让口令代换被跳过，dsh 于是直接回 401 ——
          // 且这个 cookie 永远刷不掉，连重新扫码都没用（症状：手机一直显示
          // dsh web authentication required）。丢掉旧 cookie、补上口令重试一次，
          // 让 dsh 走口令交换下发新 cookie。能走到这里的请求必已通过 gate 认证
          // （未认证请求在入口就 401），代换真口令不会送达未认证方。
