...
async function readDshToken(tokenFile) {
  if (Date.now() - tokenCache.at < 5_000) return tokenCache.value
  let value
  try {
  ...
  async function gate(req) {
    ...
    if (!authed && !out.startsWith('/plugins/')) {
      if (pathToken !== undefined) {
        ...
        const token = await readDshToken(tokenFile)
        if (token !== undefined) {
          // 移除可能残留的旧 token 参数（避免同名参数让 DSH 取到过期值）
          out = out.replace(/([?&])token=[^&]*/g, '$1')
      ...
      (upstreamRes) => {
        ...
        if (gated.remember) {
          outHeaders = mergeSetCookie(
            outHeaders,
            `${REMEMBER_COOKIE}=${encodeURIComponent(accessKey)}; Path=/; HttpOnly; Max-Age=31536000; SameSite=Strict`,
          )
        }
        ...
        upstreamRes.on('end', () => {
          const body = injectPolyfill(Buffer.concat(chunks).toString('utf8'))
