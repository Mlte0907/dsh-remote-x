import { a as REMEMBER_COOKIE_MAX_AGE, i as LOG_TAIL_BYTES, o as SOCKET_TIMEOUT, r as KEEPALIVE_MSECS } from "./constants-Zi7Pzbpq.mjs";
...
var UpstreamAgent = class {
	constructor({ keepAlive = true, maxSockets = 16, keepAliveMsecs = KEEPALIVE_MSECS } = {}) {
	...
	getConnectionStats() {
		...
		for (const key of Object.keys(freeSockets ?? {})) free += freeSockets[key]?.length ?? 0;
...
function startRemoteProxy(options) {
	...
	async function gate(req) {
		...
		const keyOk = accessKey !== void 0 && (keyMatch !== null && decodeURIComponent(keyMatch[1]) === accessKey || pathKey === accessKey);
		let tokenOk = false;
		if (!keyOk && !authed && !remembered) {
			...
			if (provided !== void 0 && provided.length > 0) {
				...
				tokenOk = latest !== void 0 && provided === latest;
			}
